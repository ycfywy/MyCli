...
- **文件引用**：输入 `@` 可浏览工作目录文件，支持 `Tab`、`↑/↓` 选择并引用文件/目录。
...
在普通输入里输入 `@` 会显示当前工作目录下的文件/目录候选，可以用 `Tab` 或 `↑/↓` 选择。
...
解释这个文件 @src/agent.ts
帮我看一下这个目录 @src/
对比这两个文件 @src/agent.ts @src/input-handler.ts
```

引用规则：

- 文本文件会作为上下文附加到本轮用户消息。
- 目录会附加目录树，默认最多递归 3 层、200 项。
- 图片文件会转成多模态图片输入。
- 二进制文件会跳过内容并提示。
- 默认补全会忽略 `node_modules/`、`.git/`、`dist/` 等目录。
...
│   ├── commands.ts       # 命令处理
│   ├── config.ts         # 配置管理
│   ├── input-handler.ts  # 输入、命令补全、@ 文件补全、选择器
│   ├── file-references.ts # @ 文件/目录引用解析
│   ├── renderer.ts       # 终端渲染
│   ├── skills.ts         # Skills 加载与注入
│   └── types.d.ts        # 类型声明
├── skills/               # 项目级 Skills，可选
├── .env.example          # 配置模板
├── claude.md             # 给 AI/Agent 阅读的项目说明
├── package.json
└── tsconfig.json
```
